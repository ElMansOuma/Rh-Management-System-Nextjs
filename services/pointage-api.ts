...
import { PointageRequest, PointageEntry, UserInfo } from '@/types/pointage';
...
const pointageService = {
    ...
    async getAllPointages(month?: string, year?: number): Promise<PointageEntry[]> {
        ...
        if (!token || !userInfo.cin) {
            throw new Error('Authentification invalide. Veuillez vous reconnecter.');
        }

        try {
            const url = new URL(`${API_URL}/api/pointage/admin`);

            if (month) {
                url.searchParams.append('month', month.padStart(2, '0'));
            }
            if (year) {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                throw new Error(errorText || 'Impossible de récupérer les pointages');
            }
